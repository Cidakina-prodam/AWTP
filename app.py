...
def injetar_notificacoes():
    tarefas_json = json.dumps(st.session_state.tarefas, ensure_ascii=False)
    html = f"""
<script>
(function(){{
  ...
  function deveAvisar(t){{
    ...
    if(t.tipo==='recorrente'){{
      ...
      if(t.recorrencia==='quinzenal'&&t.diaMes){{
        var ref=new Date(t.diaMes+'T00:00:00');
        var hjd=new Date(hoje+'T00:00:00');
        var diff=Math.round((hjd-ref)/86400000);
        return diff>=0&&diff%14===0;
      }}
      if(t.recorrencia==='mensal'&&t.diaMes){{
        var ref=new Date(t.diaMes+'T00:00:00');
        return ref.getDate()===new Date().getDate();
      }}
    }}
    return false;
  }}
  ...
  function tocarSom(){{
    try{{
      ...
      function bip(freq,t0,dur){{
        ...
        g.gain.linearRampToValueAtTime(0.35,ctx.currentTime+t0+0.02);
        g.gain.linearRampToValueAtTime(0,ctx.currentTime+t0+dur);
        o.start(ctx.currentTime+t0);o.stop(ctx.currentTime+t0+dur+0.1);
      }}
  ...
  function mostrarToast(titulo, subtitulo){{
    ...
    el.id='gt-toast';
    el.style.cssText='position:fixed;bottom:24px;right:24px;background:#fff;border:1.5px solid #e08a00;border-left:5px solid #e08a00;border-radius:10px;padding:16px 18px;max-width:320px;z-index:99999;box-shadow:0 8px 32px rgba(0,0,0,.22);font-family:-apple-system,sans-serif';
    el.innerHTML=
      '<div style="display:flex;gap:12px;align-items:flex-start">'+
        '<span style="font-size:24px;flex-shrink:0">🔔</span>'+
        '<div style="flex:1;min-width:0">'+
          '<div style="font-size:10px;font-weight:700;color:#854f0b;text-transform:uppercase;letter-spacing:.06em;margin-bottom:3px">Lembrete · '+hora+'</div>'+
          '<div style="font-size:14px;font-weight:600;color:#18180f">'+titulo+'</div>'+
          (subtitulo?'<div style="font-size:12px;color:#5a5a50;margin-top:2px">'+subtitulo+'</div>':'')+
        '</div>'+
        '<button id="gt-close" style="border:none;background:none;cursor:pointer;color:#9a9a8e;font-size:22px;line-height:1;padding:0;margin-left:4px">×</button>'+
      '</div>';
    document.body.appendChild(el);
    document.getElementById('gt-close').onclick=function(e){{e.stopPropagation();el.remove();}};
    // Notificação nativa (funciona no Streamlit Cloud por ser https)
    if(Notification&&Notification.permission==='granted'){{
      try{{ new Notification('🔔 '+titulo,{{body:subtitulo||titulo}}); }}catch(e){{}}
    }}
  }}
  ...
  function checar(){{
    ...
    tarefas.forEach(function(t){{
      ...
      var hora=t.lembreteHora||'08:00';
      var partes=hora.split(':');
      var alvo=parseInt(partes[0])*60+parseInt(partes[1]);
      if(atual<alvo||atual>alvo+3) return;
      var key=t.id+'_'+hoje+'_'+hora;
      if(fired[key]) return;
      fired[key]=true;
      localStorage.setItem('gt_fired',JSON.stringify(fired));
...
injetar_notificacoes()

if st.session_state.get("testar_notif"):
    t_teste = next((t for t in st.session_state.tarefas if t.get("lembrete")), None)
    titulo_t = t_teste["titulo"] if t_teste else "Teste de aviso"
    desc_t = t_teste.get("desc","") if t_teste else "Som e aviso visual funcionando!"
    st.components.v1.html(f"""
    <script>
    function tocarSom(){{
      try{{
        var ctx=new(window.AudioContext||window.webkitAudioContext)();
        function bip(f,t,d){{var o=ctx.createOscillator(),g=ctx.createGain();o.connect(g);g.connect(ctx.destination);o.type='sine';o.frequency.value=f;g.gain.setValueAtTime(0,ctx.currentTime+t);g.gain.linearRampToValueAtTime(0.35,ctx.currentTime+t+0.02);g.gain.linearRampToValueAtTime(0,ctx.currentTime+t+d);o.start(ctx.currentTime+t);o.stop(ctx.currentTime+t+d+0.1);}}
        bip(880,0,0.15);bip(880,0.2,0.15);bip(1100,0.4,0.25);
      }}catch(e){{}}
    }}
    function mostrarToast(titulo,sub){{
      var old=document.getElementById('gt-toast');if(old)old.remove();
      tocarSom();
      var hora=new Date().toLocaleTimeString('pt-BR',{{hour:'2-digit',minute:'2-digit'}});
      var el=document.createElement('div');el.id='gt-toast';
      el.style.cssText='position:fixed;bottom:24px;right:24px;background:#fff;border:1.5px solid #e08a00;border-left:5px solid #e08a00;border-radius:10px;padding:16px 18px;max-width:320px;z-index:99999;box-shadow:0 8px 32px rgba(0,0,0,.22);font-family:-apple-system,sans-serif';
      el.innerHTML='<div style="display:flex;gap:12px;align-items:flex-start"><span style="font-size:24px;flex-shrink:0">🔔</span><div style="flex:1"><div style="font-size:10px;font-weight:700;color:#854f0b;text-transform:uppercase;letter-spacing:.06em;margin-bottom:3px">Lembrete · '+hora+'</div><div style="font-size:14px;font-weight:600;color:#18180f">'+titulo+'</div>'+(sub?'<div style="font-size:12px;color:#5a5a50;margin-top:2px">'+sub+'</div>':'')+' </div><button id="gt-close" style="border:none;background:none;cursor:pointer;color:#9a9a8e;font-size:22px;line-height:1;padding:0;margin-left:4px">×</button></div>';
      document.body.appendChild(el);
      document.getElementById('gt-close').onclick=function(e){{e.stopPropagation();el.remove();}};
    }}
    mostrarToast("{titulo_t}","{desc_t}");
    </script>
    """, height=0)
    st.session_state.pop("testar_notif", None)
